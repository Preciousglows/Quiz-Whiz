const questions = [
  ...
  },
  {
    ...
    options: [
      "A programming language for creating interactive websites",
      "A type of coffee drink popular among developers",
...
// Load score history from browser storage
...
function renderQuestion() {
  let elem = `
  <meter id="quiz-meter" class="quiz-meter" max= "100" value ="0"></meter>
  ...
  <div class="question-container">
   ...
   <ul class="quiz-ul">
   <li class="quiz-li">
       <label for="option-1" class="quiz-label">${
         randomObject.options[0]
       }</label>
       <input type="radio" name="quiz-option" id="option-1" value="${
         randomObject.options[0]
       }">
   </li>
       
   <li class="quiz-li">
       <label for="option-2" class="quiz-label">${
         randomObject.options[1]
       }</label>
       <input type="radio" name="quiz-option" id="option-2" value="${
         randomObject.options[1]
       }">
   </li>
       
   <li class="quiz-li">
       <label for="option-3" class="quiz-label">${
         randomObject.options[2]
       }</label>
       <input type="radio" name="quiz-option" id="option-3" value="${
         randomObject.options[2]
       }">
   </li>
       
   <li class="quiz-li">
       <label for="option-4" class="quiz-label">${
         randomObject.options[3]
       }</label>
       <input type="radio" name="quiz-option" id="option-4" value="${
         randomObject.options[3]
       }">
   </li>
   </ul>
...
function progressMeter(){
  let quizMeter = document.querySelector('#quiz-meter');
  let percentage = ((counter+1) /questions.length) * 100;
  quizMeter.value = percentage;
}
...
function renderSummary() {
  let elem = `
    ...
    <div class="summary-body">
      <p class="summary-p">You answered ${store} out of ${
    questions.length
  } questions</p>
      <h2 class="summary-h2">Your score is ${score} over ${
    questions.length
  }</h2>
      ...
      <h3>Score History (last 5 attempts):</h3>
      <ul>
        ${scoreHistory.map((score, index) => `<li>Attempt ${index + 1}: ${score} out of ${questions.length}</li>`).join("")}
      </ul>
    </div>
    </div>
  `;
